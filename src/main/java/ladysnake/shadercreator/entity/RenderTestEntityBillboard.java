...
public class RenderTestEntityBillboard extends Render<TestEntity> {
    ...
    public void doRender(@Nonnull TestEntity entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        ShaderUtil.setUniform("playerPos", new float[] {(float)player.posX, (float)player.posY + player.eyeHeight, (float) player.posZ});
        ShaderUtil.setUniform("centerPos", new float[] {(float)entity.posX, (float)entity.posY + entity.height / 2, (float)entity.posZ});
        ShaderUtil.setUniform("radius", 3);
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_NORMAL);
        bufferbuilder.pos(-1D, -0.5D, 0.0D).tex(0D, 0D).normal(0.0F, 1.0F, 0.0F).endVertex();
        bufferbuilder.pos(1D, -0.5D, 0.0D).tex(0D, 1D).normal(0.0F, 1.0F, 0.0F).endVertex();
        bufferbuilder.pos(1D, 1.5D, 0.0D).tex(1D, 1D).normal(0.0F, 1.0F, 0.0F).endVertex();
        bufferbuilder.pos(-1D, 1.5D, 0.0D).tex(1D, 0D).normal(0.0F, 1.0F, 0.0F).endVertex();
        tessellator.draw();
        ShaderUtil.revert();
